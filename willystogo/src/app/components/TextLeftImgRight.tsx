...
const TextLeftImgRight: React.FC<TextLeftImgRightProps> = ({ documentId, images }) => {
  ...
  return (
    <div ref={container} className="mt-0 sm:mt-10 pl-8 sm:pl-24 min-h-[500px] grid grid-cols-1 md:grid-cols-5 gap-4 p-4 sm:p-16">
      <div className="header-shadow-left col-span-3 bg-secondary flex px-10 pt-4">
        <EditorWrapper documentId={documentId} />
      </div>
      <div className="relative col-span-2 flex flex-col justify-between">
        <motion.div style={{ y: useTransform(scrollYProgress, [0, 1], [0, -dimension.height * 0.2]) }} className="w-full mt-24 h-[60%]">
          <Image src={images[0]} alt="Image 1" layout="fill" className="object-cover object-center" />
        </motion.div>
        <motion.div style={{ y: useTransform(scrollYProgress, [0, 1], [0, dimension.height * 0.2]) }} className="w-full h-[60%]">
